...
struct RetroDMGDebugView: View {
    ...
    var body: some View {
        RetroDMGViewPort {
            RetroDMGDebugMetalView(core: core)
        }
...
class RetroDMGDebugMetalView: MTKView {
    var renderer: RetroDMGDebugRenderer!
    var core: RetroDMG!
    ...
    func createRenderer(device: MTLDevice){
        renderer = RetroDMGDebugRenderer(device: device, core: self.core)
        delegate = renderer
    }
}

// MARK: Renderer
class RetroDMGDebugRenderer: NSObject {
...
extension RetroDMGDebugRenderer: MTKViewDelegate {
    func mtkView(_ view: MTKView, drawableSizeWillChange size: CGSize) {
        view.clearColor = MTLClearColor(red: 0.804, green: 0.859, blue: 0.878, alpha: 1)
    }
    
    func draw(in view: MTKView) {
        let pixels =  core.tileData(rowCount: 24, columnCount: 16)
        ...
        for pixel in 0..<pixels.count {
            ...
            let tilesPerRow: Float = 24
            let rowsPerScreen: Float = 16
