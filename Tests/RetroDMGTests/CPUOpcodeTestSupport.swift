...
// Shared support for CPU opcode JSON-based tests
public struct CPUOpcodeTestSupport {
    public static func testFiles() throws -> [CPUJsonTest] {
        let testFileNames = [
        ...
        for test in testFileNames {
            if let url = Bundle.module.url(forResource: test, withExtension: "json") {
                do {
                    let file = try Data(contentsOf: url)
                    testFiles.append(contentsOf: try JSONDecoder().decode([CPUJsonTest].self, from: file))
                } catch {
                    // Optionally log or handle decode error
                }
            } else {
...
public struct CPUJsonTests: Codable {
    public let tests: [CPUJsonTest]
}

public struct CPUJsonTest: Codable {
    ...
    public struct State: Codable {
        ...
        public let h: UInt8
        public let ie: UInt8?
        public let ime: UInt8?
        public let l: UInt8
        public let pc: UInt16
        public let ram: [[UInt16]]
