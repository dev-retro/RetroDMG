...
struct TestROMIntegrationTests {
    ...
    @Test("Test harness basic functionality")
    func testTestHarnessBasic() throws {
        // Just verify we can access a test harness class
        #expect(TestHarness() != nil, "TestHarness should be accessible")
        print("Test harness accessibility test passed")
    }

    @Test("Blargg CPU instruction test: framebuffer matches reference PNG")
    func testBlarggCPUInstrsFramebuffer() async throws {
        let romPath = "Resources/blargg/cpu_instrs/cpu_instrs.gb"
        let referencePNGPath = "Resources/blargg/cpu_instrs/cpu_instrs-dmg-cgb.png"
        ...
        let config = TestHarness.TestConfig(
            duration: 70, // Run for 70 seconds (adjust as needed)
            maxFrames: 100,
